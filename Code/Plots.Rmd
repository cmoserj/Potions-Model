...
path<-('K:/git/Potions-Model/Results/')
...
scatcomb<-ggplot()+
  geom_point(data=scatter,aes(y=CombTime, x=Gini,color=Network),alpha=0.3)+
  ...
  theme(plot.title=element_text(hjust=0.5,size=20,margin=margin(b=10)),
        ...
        aspect.ratio=1,
        legend.title.align = 0.5)+
  labs(title="Combinations to Crossover",
       y="",
      color="Network",
      x="Gini Coefficient (Scaled)")+
  scale_color_viridis(discrete=T, direction = -1)
...
SIFig3<-scatstep+scatcomb+
  plot_layout(design=scatlayout)+
  plot_annotation(title = 'Performance vs Gini\nAll Networks, All Parameters', 
